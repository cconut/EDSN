...
def parse_args():
    ...
    parser.add_argument('--model_name', default='EDSN', choices=['UNet', 'UKan', 'EDSN'], help='model name')
...
def main():
    ...
    config_path = r'/root/autodl-tmp/output_recons/models/MitoMts_EDSN/tmp_res_1/config.yml'
    ...
    if config['model_name'] == 'UNet':
        ...
        model = EDSN(n_channels=config['input_channels'], n_classes=config['num_classes'], device=device).to(device)
    else:
        raise NotImplementedError('Model not implemented')

    ckpt = torch.load('/root/autodl-tmp/output_recons/models/MitoMts_EDSN/model_final/epoch_80.pth')
