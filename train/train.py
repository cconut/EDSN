...
from archs.unet_rcan import UNet_RCAN
...
def parse_args():
    ...
    # model
    parser.add_argument('--model_name', default='UNet_RCAN',
                        choices=['UNet', 'UKan', 'EDSN'],)
...
def main():
    ...
    elif config['model_name'] == 'UNet_RCAN':
        model = UNet_RCAN(n_channels=config['input_channels'], n_classes=config['num_classes']).to(device)
    elif config['model_name'] == 'EDSN':
        model = EDSN(n_channels=config['input_channels'], n_classes=config['num_classes']).to(device)
    else:
